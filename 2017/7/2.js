...
const selfRegex = /(\w+)\s*\((\d+)\)/;
function parseSelf(selfText) {
  const match = selfText.trim().match(selfRegex);

  if (!match) {
    throw new Error('cannot parse self:' + selfText);
  }

  const [__, name, weight] = match;

  return { name, weight };
}

const trimString = String.prototype.trim;
const othersDelimiterRegex = /,\s*/;
function parseOthers(othersText) {
  return othersText.trim().split(othersDelimiterRegex);
}

function parseRow(rowText) {
  const [selfText, othersText] = rowText.split('->');

  const self = parseSelf(selfText);

  const record = { self };

  if (othersText) {
    record.others = parseOthers(othersText);
  }

  return record;
}

function parsePrograms(programsText) {
  const programRows = programsText.split('\n');

  const programs = programRows.map(parseRow);

  return programs;
}

function programsByName(programs) {
  const result = {};
  for (const p of programs) {
    result[p.self.name] = p;
  }
  return result;
}

function getOthersWeight(programsByName) {}

function findBottom(programs) {
  const beingHeld = new Set();
  const isHolding = [];

  for (const p of programs) {
    console.log(p);
    if (p.others) {
      isHolding.push(p.self.name);
  ...
  for (const holder of isHolding) {
    if (!beingHeld.has(holder)) {
      return holder;
    }
  }
}

console.log(findBottom(parsePrograms(input)));
