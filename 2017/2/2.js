...
function parseRow(rowText) {
  return rowText.trim().split(whitespaceRegex).map(cellText => parseInt(cellText, 10));
}

function diffLargestSmallest(rowText) {
  const elements = parseRow(rowText);
...
function findEvenlyDivisiblePair(elements) {
  for (let i = 0; i < elements.length - 1; i++) {
    for (let j = i + 1; j < elements.length; j++) {
      const pair = sortPair(elements[i], elements[j]);
      const [a, b] = pair;
      if (b % a === 0) {
        return pair;
      }
...
function getRowValue(rowText) {
  const elements = parseRow(rowText);
  const [a, b] = findEvenlyDivisiblePair(elements);
...
function checkSum(spreadsheet) {
  ...
  return R.sum(rows.map(getRowValue));
}
