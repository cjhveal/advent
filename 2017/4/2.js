...
function boolToNumber(x) {
  return x ? 1 : 0;
}

function countValidPhrases(phrases) {
  const counts = phrases.map(isValid).map(boolToNumber);
  return R.sum(counts);
}

function parsePhrases(phrasesText) {
  return phrasesText.split('\n').map(phrase => phrase.split(' '));
}

function test(text) {
  console.log(countValidPhrases(parsePhrases(text)));
}
