...
class Game {
  ...
  static parse(gameText: string): Game {
    ...
    const results = resultTexts.map((resultText) => {
      return GameResult.parse(resultText)
    });
  ...
  getMinimumRequired(): GameResult {
    ...
    return new GameResult(maxR, maxG, maxB);
  }
}
...
function main(filename: string) {
  ...
  for (const game of games) {
    const minResult = game.getMinimumRequired();
    total += minResult.getPower();
  }
