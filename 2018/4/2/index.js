const INPUT = `[1518-10-31 00:58] wakes up
...
function parseLog(rawLog) {
  const match = rawLog.match(LOG_REGEX);

  if (!match) {
    throw new Error('unable to parse log' + rawLog);
  }

  const [fullMatch, timestamp, message] = match;
...
const MESSAGE_REGEX = /Guard #(\d+)/;


function buildSleepIntervals(logs) {
  let currentGuard = null;

  const events = [];

  for (const log of logs) {
    if (log.message === 'wakes up') {
      events.push({ guard: currentGuard, state: 'awake', log });
    } else if (log.message === 'falls asleep') {
      events.push({ guard: currentGuard, state: 'sleep', log });
    } else {
      const match = log.message.match(MESSAGE_REGEX);
      if (!match) {
        throw new Error('could not parse message');
      }
      const [allMatch, guardId] = match;
  ...
  const intervals = [];
  let sleepEvent = null;
  currentGuard = events[0].guard;

  for (const e of events) {
    if (!sleepEvent && e.state === 'sleep') {
      sleepEvent = e;
    } else if (sleepEvent && e.state === 'awake') {
      const start = sleepEvent.log.minute;
      const end = e.log.minute;

      intervals.push({
        guard: sleepEvent.guard,
        start: sleepEvent.log,
        end: e.log,
      });
      sleepEvent = null;
    }
...
function groupByGuardAndSum(intervals) {
  return intervals.reduce((map, interval) => {
    const len = interval.end.minute - interval.start.minute;
    map[interval.guard] = (map[interval.guard] || 0) + len
    return map;
  }, {})
}

function makeHistogramByGuard(intervals) {
  const guards = {};

  for (const interval of intervals) {
    if (guards[interval.guard] == undefined) {
      guards[interval.guard] = Array.from({ length: 60 }).fill(0);
    }

    const histo = guards[interval.guard];
    const start = interval.start.minute;
    const end = interval.end.minute;
...
function findMaxGuardAndMinute(histograms) {
  let maxSoFar = 0;
  let maxGuard = null;
  let maxMinute = null;
  for (const [guard, histogram] of Object.entries(histograms)) {
    for (const [minute, count] of Object.entries(histogram)) {
      if (count > maxSoFar) {
        maxSoFar = count;
        maxGuard = guard;
        maxMinute = minute;
...
function processLogs(input) {
  ...
  const sleepIntervals = buildSleepIntervals(logs);

  const histograms = makeHistogramByGuard(sleepIntervals);

  const { guard, minute } = findMaxGuardAndMinute(histograms);


  return Number(guard) * minute;
}

console.log(processLogs(INPUT));
