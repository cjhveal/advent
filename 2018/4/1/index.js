...
function makeHistogram(intervals) {
  const histo = Array.from({ length: 60 }).fill(0);
...
function processLogs(input) {
  ...
  const guardSums = groupByGuardAndSum(sleepIntervals);

  const maxGuard = findMaxKey(guardSums);

  const chosenIntervals = sleepIntervals.filter(i => i.guard === maxGuard);

  const guardHistogram = makeHistogram(chosenIntervals);

  const chosenMinute = findMaxKey(guardHistogram);
  console.log(guardHistogram);
  console.log(chosenMinute);

  return chosenMinute * Number(maxGuard);
}
