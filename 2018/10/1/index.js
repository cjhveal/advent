const INPUT = `position=< 50769, -40375> velocity=<-5,  4>
...
const PARTICLE_REGEX = /position=<\s*(-?\d+),\s*(-?\d+)>\s+velocity=<\s*(-?\d+),\s*(-?\d+)>/;
function parseParticle(raw) {
  const match = raw.match(PARTICLE_REGEX);

  if (!match) {

  }

  const [fullMatch, px, py, vx, vy] = match;
...
function processParticles(input) {
  const rawParticles = input.split('\n');

  const particles = rawParticles.map(parseParticle);

  const system = new System(particles);
...
class System {
  constructor(particles) {
    this.particles = particles.map(p => Object.assign({}, p));
  }
  ...
  getBounds() {
    const xs = this.particles.map(p => p.px);
    const width = Math.max(...xs);
    const x0 = Math.min(...xs);

    const ys = this.particles.map(p => p.py);
  ...
  print() {
    const { width, height, x0, y0 } = this.getBounds();
    const grid = Array(height - y0 + 1).fill().map(() => Array(width - x0 + 1).fill('.'));

    console.log(this.getBounds(), grid[0].length)
    this.particles.forEach(p => {
      console.log(p);
