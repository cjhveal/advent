...
function normalizeRange(range, points) {
  const {xMin, xMax, yMin, yMax} = range;

  const normalizedRange = [xMax - xMin, yMax - yMin];

  const normalizedPoints =  points.map(([x, y]) => ([ x - xMin, y - yMin ]));

  return  [normalizedRange, normalizedPoints];
}

const serializePoint = ([x, y]) => {
  return `${x},${y}`;
}
...
function main() {
  ...
  for (const [point, color] of painter.panelBank.entries()) {
    if (color === 1) {
  ...
  const deserializedPoints = paintedPoints.map(x => x.split(',').map(Number));

  
  const range = getPointRange(deserializedPoints);

  const [normalizedRange, normalizedPoints] = normalizeRange(range, deserializedPoints);

  const normalPointSet = new Set(normalizedPoints.map(serializePoint));


  const [xMax, yMax] = normalizedRange;
