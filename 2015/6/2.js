...
var cmdRegex = /(turn on|turn off|toggle) (\d+),(\d+) through (\d+),(\d+)/;

function parse(cmd) {
  const match = cmd.match(cmdRegex);

  const [full, action, x1, y1, x2, y2] = match
  return {
    action,
    x1: parseInt(x1, 10),
    y1: parseInt(y1, 10),
...
console.log(parse('toggle 0,0 through 0,999'));

const LitMap = new Map();

const inc = (pos, n=1) => {
  const val = LitMap.get(pos) || 0;
  LitMap.set(pos, val + n);
}

const dec = (pos) => {
  const val = LitMap.get(pos) || 0;
  LitMap.set(pos, Math.max(0, val - 1))
}
...
const handlers = {
  'turn on': (pos) => inc(pos),
  'turn off': (pos) => dec(pos),
  'toggle': (pos) => inc(pos, 2)
}

function handle(action, x, y) {
  const pos = serialize(x,y);
...
for (let input of data) {
  ...
  for (let x = cmd.x1; x <= cmd.x2; x++) {
    for (let y = cmd.y1; y <= cmd.y2; y++) {
      handle(cmd.action, x, y)
    }
  }
}

let sum = 0
LitMap.forEach(val => sum += val)
console.log(sum);
