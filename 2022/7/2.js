...
function main(inputFile) {
  ...
  const sizeUsed = parser.root.size();

  const sizeRemaining = TOTAL_SIZE - sizeUsed;

  const sizeToRemove = TARGET_SIZE - sizeRemaining;


  let current = parser.root.size();
  parser.root.walk((dir) => {
    const size = dir.size();
    if (size >= sizeToRemove && size < current) {
      current = size;
    }
  });
